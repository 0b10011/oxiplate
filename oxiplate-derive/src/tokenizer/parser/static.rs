...
use crate::tokenizer::{Context, TagKind, Token, TokenKind, WhitespacePreference, whitespace};

pub fn consume_static_whitespace<'a>(
    ...
    (None, Token::new(TokenKind::StaticWhitespace, &source, None))
}

#[allow(clippy::unnested_or_patterns)]
pub fn consume_static_text<'a>(source: &mut BufferedSource<'a>) -> (Option<Context>, Token<'a>) {
    let source = source
        .consume_until(|char| matches!(char, '{' | whitespace!()))
        .expect("Buffer should contain at least one character");

    (None, Token::new(TokenKind::StaticText, &source, None))
}

pub fn consume_possible_tag_start<'a>(
    source: &mut BufferedSource<'a>,
) -> (Option<Context>, Token<'a>) {
    let (new_context, kind) = match source.peek() {
        ...
        _ => {
            ...
            return (None, Token::new(kind, &source, None));
        }
    };

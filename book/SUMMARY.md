# Summary

[Introduction](README.md)

# Setting up the rust project

- [Getting started](getting-started.md)

# Building templates

- [Template introduction](templates/README.md)
- [Tags](templates/tags.md)
- [Whitespace control](templates/whitespace-control.md)
- [Writs](templates/writs/README.md)
    - [Escaping](templates/writs/escaping.md)
- [Statements](templates/statements/README.md)
    - [default_escaper_group / replace_escaper_group](templates/statements/default-escaper-group.md)
    - [extends / block / parent](templates/statements/extends.md)
    - [include](templates/statements/include.md)
    - [if / elseif / else](templates/statements/if-else.md)
    - [for / continue / break / else](templates/statements/for.md)
    - [match / case](templates/statements/match.md)
    - [let](templates/statements/let.md)
- [Expressions](templates/expressions.md)
